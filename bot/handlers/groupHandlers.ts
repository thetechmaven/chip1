  ...
  try {
    ...
    const isBotBeingAdded =
      message.new_chat_members?.some(
        (member) => member.id === bot.options.id
      ) ||
      ...
      if (isNewGroup || isBotBeingAdded) {
        ...
        try {
          ...
          if (
            ...
            console.log(
              'Context length exceeded, trying with shorter context...'
            );

            // Try again with just the current message and shorter knowledgebase
            try {
              const shortenedKnowledgebase = knowledgebase
                ? knowledgebase.split('\n').slice(0, 5).join('\n')
                : '';
              responseText = await dealOpenaiWrapper(
                getChatId(message),
                [
                  {
                    role: 'user',
    ...
    try {
      ...
      console.log(
        ...
        editError && typeof editError === 'object'
          ? (editError as { message?: string }).message
          : editError
      );
      try {
        await bot.sendMessage(getChatId(message), responseText, {
          parse_mode: 'Markdown',
        });
      } catch (sendError) {
        console.error(
