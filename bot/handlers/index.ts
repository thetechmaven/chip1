...
let handlers: (bot: typeof TelegramBot) => void;
if (process.env.ENV === 'test') {
  handlers = (bot: typeof TelegramBot) => {
    ...
    return async (msg: TelegramBotTypes.Message) => {
      ...
      if (currentCommand && commandHandlers[currentCommand]) {
        return commandHandlers[currentCommand]({
          bot,
        ...
        return handleOther({ bot, message: msg, command: '' });
      }
    };
  };
} else {
